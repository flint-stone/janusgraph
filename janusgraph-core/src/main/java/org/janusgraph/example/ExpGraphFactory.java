...
public class ExpGraphFactory {
    ...
    //static String _prefix_path = "/home/lexu/codelab/RelationReader";
    static String _prefix_path = "/Users/lexu/Projects/docker-clion-dev/codelab/social_network/";
    ...
    static void get_vertex_files() {
        ...
        if(vertexLabels.isEmpty()) {
            for(String file_name : _cur_files) {
            	String vertex_label = _name_map.get(file_name.split("/social_network/")[1].split("_0_0")[0]);
    ...
    static void get_edge_files() {
        ...
        if(edgeLabels.isEmpty()) {
            for(String file_name : _cur_files) {
            	String relation_name = file_name.split("/social_network/")[1].split("_0_0")[0];
   ...
   static void load_vertex(JanusGraph graph) {
	   get_vertex_files();
        for(String file_name : _cur_files) {
     	    int count =0;
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(file_name))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    //long janusVertexId = ((StandardJanusGraph) graph).getIDManager().toVertexId(id);
    ...
    static void load_edges(JanusGraph graph) {
        get_edge_files();
        for(String file_name : _cur_files) {
            int count =0;
            JanusGraphTransaction tx = graph.newTransaction();
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(file_name))) {
                ...
                line = br.readLine();
                while ((line = br.readLine()) != null) {
                    long local_src = Long.valueOf(line.split("\\|")[0]);
                    long local_dest = Long.valueOf(line.split("\\|")[1]);
 ...
 public static void load(final JanusGraph graph) {
	 ...
	 for(String edge_label : edgeLabels) {
		 management.makeEdgeLabel(edge_label).make();
	 }
	 for(String vertex_label : vertexLabels) {
